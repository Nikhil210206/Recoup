...
| Action choice | Real code, synthetic input | **Real** — `method_switch_prompt` |
| Execution | Simulated | **Real** Razorpay payment link |
| Outcome | **Simulated** from planted recovery parameters | **Real** payment, cross-verified `paid` |

Every headline number in this README comes from the left-hand column and is a
simulation benchmark, not production Razorpay data. The right-hand column is one
payment, evidenced in [`docs/evidence/`](docs/evidence/).

Recovery probabilities are estimates. Nobody publishes conditional recovery
rates by failure cause — that is exactly the proprietary knowledge a payments
company accumulates — so the simulator cannot validate the model. It validates
the *allocator*, which is why the probability oracle is degraded deliberately
rather than trusted. The full limits are in
...
**₹176,142 is deliberately not chased.** All 27 of those cases are
`risk_blocked` — stopped by a risk or fraud check, where the taxonomy records
`who_can_fix = nobody`. If the block is correct, recovering the payment means
recovering a fraudulent one; if it is a false positive, no customer nudge clears
...
somebody can fix it and it is not the payer. "Nobody can act" and "the customer
cannot act" are different instructions, and only the first one ends in silence.
...
The LLM tail runs on a local model (Ollama, `qwen2.5:7b`). An Anthropic provider
is implemented and the switch on `ANTHROPIC_API_KEY` is covered by tests, but
**that path has never executed against the live API** — there is no key. Every
LLM measurement quoted here is Ollama's. Model choice was decided by a held-out
evaluation, not preference: a 3B model scored 4/7 with **three confidently-wrong
answers above the acting threshold**.
...
number that shows a gate is load-bearing is how often it *refused*. On the base
world's 2,400-case test split, 1,893 were acted on and the gates declined:

| reason | rule | cases | refused |
|---|---|---|---|
| cause is not recoverable by the customer | `risk_suppression` | 27 | ₹176,142 |
| expected value below the floor | `ev_floor` | 22 | ₹6,531 |
| customer already at their contact cap | `contact_budget` | 1 | ₹2,232 |
| **total** | | **50** | **₹184,905** |

A further 457 cases (₹296,894) went unworked because the contact budget ran out.
**That is not a gate refusing and the two are never summed** — a governance
refusal would still refuse with unlimited budget, whereas exhaustion moves with
the budget rather than with policy. Adding them gives a larger number that
measures neither. Section 8 of [`EVALUATION.md`](EVALUATION.md) reports both
lines, generated by `make eval`.

## What this cannot tell you

- Recovery probabilities are **estimates**, labelled as such in
  [`data/ASSUMPTIONS.md`](data/ASSUMPTIONS.md), and the simulator **cannot
  validate the model** — the generating process is known. Both are stated at the
  top of this file as well, because they bound every number below them.
