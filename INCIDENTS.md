...
Setup day. Two things broke: one environment problem worth recording because of
how it disguised itself, and the first real webhook, which arrived carrying a
failure class I had not planned for.

---

### A port collision that presented as a credentials error

**Expected:** `make db` starts Postgres on 5433, integration tests connect.

**Actually:** the container failed with `Bind for 0.0.0.0:5433 failed: port is
...
**Why:** I had picked 5433 to dodge a local Postgres already on 5432. But
another project of mine was already bound to 5433. So the container never
